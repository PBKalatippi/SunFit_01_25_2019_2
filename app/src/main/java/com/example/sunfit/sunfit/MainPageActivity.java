...
public class MainPageActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        final TextView TotalRedeemedStepsTextView = (TextView) findViewById(R.id.textView_total_redeemed_steps);
        final TextView StepsAvailRedeemTextView = (TextView) findViewById(R.id.textView_steps_available_for_redeem);
        final ImageView QRImageView = (ImageView) findViewById(R.id.imageView_QR);
        if(requestCode == 1){
            if(resultCode == RESULT_OK){
                long v_redeemed_activity_steps=data.getLongExtra("REDEEMED STEPS",0);
                String v_redeemed_activity_steps_string=Long.toString(v_redeemed_activity_steps);
                ...
                if(v_redeemed_activity_steps > 0) {
                    MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
                    try {
                        BitMatrix bitMatrix = multiFormatWriter.encode(v_redeemed_activity_steps_string, BarcodeFormat.QR_CODE, 500, 500);
